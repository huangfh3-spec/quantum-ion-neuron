...
# 布居数实验数据
def exp_population_data(filename="High_Low_data.csv"):
    path = DATA / filename
    return read_three_column_csv(path)
...
# 2) 读取布居数实验数据
time_pop, pop_exp, pop_err = exp_population_data("High_Low_data.csv")
...
# 设置主图坐标轴和格式
ax_main.set_xlabel('Time (μs)', labelpad=5)
ax_main.set_ylabel(r'$\mathit{V}_{\mathrm{q}}$', fontweight='bold', labelpad=5)
...
ax_inset.set_xlabel('Time (μs)', fontsize=22)
formatter.set_scientific(True)
formatter.set_powerlimits((-2, 2))
ax_inset.yaxis.set_major_formatter(formatter)
ax_inset.set_ylabel(r'$\mathit{V}_{\mathrm{q}}$', fontsize=22, fontweight='bold')
